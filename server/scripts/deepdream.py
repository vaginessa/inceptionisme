...
if hasGPU and (mode == "prod" or mode == "test"):
	caffe.set_device(0)
	caffe.set_mode_gpu()
...
while True:
	...
	if not hasGPU or not os.path.isfile(gpuLockPath) or mode == "secondary":
		# lock GPU
		if (hasGPU and mode != "secondary"):
			print("locking GPU!")
			...
			try:
				if file.endswith(".jpg") and os.path.isfile(imagesNew+"/"+file):
					...
					if (file.startswith("guided_")):
						...
						dreamed=deepdream(net, img, 6, end=end, objective=objective_guide)
	...
	try:
		if doDelete:
			...
			for file in os.listdir(cleanPath):
				...
				if os.stat(filepath).st_ctime < now - (5*3600):
			...
			for file in os.listdir(cleanPath):
				filepath = os.path.join(cleanPath, file)
				if os.stat(filepath).st_ctime < now - (3*24*3600):
					if os.path.isfile(filepath):
